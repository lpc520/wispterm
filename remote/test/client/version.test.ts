...
test("web version is exposed for the remote UI", () => {
  assert.equal(WEB_VERSION, "v0.28.1");
  assert.equal(webVersionLabel(), "Web v0.28.1");
});

test("web version label always includes the release version", () => {
  assert.equal(webVersionLabel("build fixture"), "Web v0.28.1 (build fixture)");
});

test("remote brand markup exposes the web version for shell views", () => {
  assert.equal(remoteBrandMarkup(), 'Phantty Remote <span class="web-version">Web v0.28.1</span>');
});
