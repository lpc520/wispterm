...
class KittyGraphicsEmitTests(unittest.TestCase):
    def test_default_moves_cursor_by_omitting_no_move_flag(self):
        ...
        emit_png(b"png", cols=4, rows=2, phantty_fallback=False, stream=out)
    ...
    def test_no_move_cursor_sets_kitty_no_move_flag(self):
        ...
        emit_png(b"png", move_cursor=False, phantty_fallback=False, stream=out)
    ...
    def test_chunk_continuations_only_set_more_flag(self):
        ...
        emit_png(b"x" * 5000, image_id=7, phantty_fallback=False, stream=out)
    ...
    def test_default_emits_phantty_osc_fallback(self):
        out = io.StringIO()

        emit_png(b"png", image_id=9, stream=out)

        self.assertIn("\x1b]7747;PhanttyImage=a=T", out.getvalue())
